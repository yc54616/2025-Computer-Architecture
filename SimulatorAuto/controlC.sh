...
# Benchmark Score 기준
benchmark_threshold=10.2

# 결과 디렉토리
RESULT_DIR="${SCRIPT_DIR}/C"
...
for file in "${RESULT_DIR}"/C_configC_*.txt; do
                ...
                for l2_associativity in "${L2_ASSOCIATIVITIES[@]}"; do
                    ...
                    if [ $total_cache_required -le $l2_cache_size ]; then
                        ...
                        for bp in "${BP_STRATEGIES[@]}"; do
                            ...
                            output_file="${RESULT_DIR}/C_configC_${l1_cache_size}_${l1_block_size}_${l1_associativity}_L2_${l2_cache_size}_${l2_block_size}_${l2_associativity}_${bp}.txt"
...
EOF
                            ...
                            "${SCRIPT_DIR}/Simulator" "${SCRIPT_DIR}/benchmark/C.riscv" -c > "$output_file" &
