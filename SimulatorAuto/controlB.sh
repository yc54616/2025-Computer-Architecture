...
# Benchmark Score 기준
benchmark_threshold=3.3

# 결과 디렉토리
RESULT_DIR="${SCRIPT_DIR}/B"
...
for file in "${RESULT_DIR}"/B_configB_*.txt; do
                ...
                for l2_associativity in "${L2_ASSOCIATIVITIES[@]}"; do
                    ...
                    if [ $total_cache_required -le $l2_cache_size ]; then
                        ...
                        for bp in "${BP_STRATEGIES[@]}"; do
                            ...
                            output_file="${RESULT_DIR}/B_configB_${l1_cache_size}_${l1_block_size}_${l1_associativity}_L2_${l2_cache_size}_${l2_block_size}_${l2_associativity}_${bp}.txt"
...
EOF
                            ...
                            "${SCRIPT_DIR}/Simulator" "${SCRIPT_DIR}/benchmark/B.riscv" -b > "$output_file" &
